$(document).ready(function () {
    ...
    if ($('.bank').length > 0){
        $('.bank').click(function () {
            ...
            $('#big_basket').fadeOut(200, function () {
                $('.dn').fadeIn(200, function () {
                    $(this).append('<p>Please wait while we are Processing Order...</p>');
                    sendBankRequest(token);
                });
            });
        });
    }

    function sendBankRequest(token){

        $.ajax({
            type: 'POST',
            url: 'modules/bank.php',
            data: {token: token},
            success: function (data) {
                if (data == 1)
                    window.location.replace('/?page=account-info');
                else
                    alert('An Error has Occurred.');
            },
            error: function (data) {
